...
/**
 * Represents the contents of a Xilinx Frame Address Register and performs
 * proper frame address register incrementing. This class stores the FAR as a
 * set of fields in the FAR rather than as a 32 bit number. This facilitates the
 * FAR incrementing.
 * 
 * This class contains many static methods that are used to evaluate Frame
 * Address information based on configuration specifications. These static
 * methods form a bridge between the values in the configuration specification
 * and the frame adddress.
 */
public class FrameAddressRegister {
	 ...
	 */
	public static int createBlockStartingFAR(
			XilinxConfigurationSpecification spec, int blockType) {
		return createFAR(spec, 0, blockType, 0, 0, 0);
	}

	/**
	 * Create an integer frame address from the various fields that make up the
	 * frame address register.
	 */
	public static int createFAR(XilinxConfigurationSpecification spec,
			int topBottom, int blockType, int row, int column, int minor) {
		int far = 0;
	...
	public static int getAddressFromBlockType(
			XilinxConfigurationSpecification spec, int blockType) {
		return blockType << spec.getBlockTypeBitPos();
	}

	public static int getAddressFromColumn(
			XilinxConfigurationSpecification spec, int column) {
		return column << spec.getColumnBitPos();
	}

	public static int getAddressFromMinor(
			XilinxConfigurationSpecification spec, int minor) {
		return (minor) << spec.getMinorBitPos();
	}

	public static int getAddressFromRow(XilinxConfigurationSpecification spec,
			int row) {
		return row << spec.getRowBitPos();
	}

	// Methods for creating FAR addresses from field values
	public static int getAddressFromTopBottom(
			XilinxConfigurationSpecification spec, int topBottom) {
		return topBottom << spec.getTopBottomBitPos();
	}

	public static int getBlockTypeFromAddress(
			XilinxConfigurationSpecification spec, int address) {
		return (address & spec.getBlockTypeMask()) >>> spec
				.getBlockTypeBitPos();
	}
	 ...
	 */
	public static int getBlockTypeNumber(XilinxConfigurationSpecification spec,
			BlockType blocktype) {

		return spec.getBlockTypes().indexOf(blocktype);
	}

	public static int getBRAMContentFrameAddress(
			XilinxConfigurationSpecification spec) {
		int blockTypeNum = getBlockTypeNumber(spec,
				spec.getBRAMContentBlockType());
		return createBlockStartingFAR(spec, blockTypeNum);
	}

	public static int getColumnFromAddress(
			XilinxConfigurationSpecification spec, int address) {
		return (address & spec.getColumnMask()) >>> spec.getColumnBitPos();
	}

	/**
	 * Return the 'consecutive' address from a FAR address. This is usually used
	 * for accessing frame data from the sequential frame data array.
	 */
	public static int getConsecutiveAddress(
			XilinxConfigurationSpecification spec, int address) {
	...
	public static int getFramesInBottom(XilinxConfigurationSpecification spec,
			int block) {
		return getFramesPerRow(spec, block) * spec.getBottomNumberOfRows();
	}

	public static int getFramesInTop(XilinxConfigurationSpecification spec,
			int block) {
		return getFramesPerRow(spec, block) * spec.getTopNumberOfRows();
	}

	public static int getFramesPerConfigurationBlock(
			XilinxConfigurationSpecification spec, int blockNum, int column) {
	...
	public static int getFramesPerFARBlockType(
			XilinxConfigurationSpecification spec, int block) {
		return getFramesInTop(spec, block) + getFramesInBottom(spec, block);
	}

	public static int getFramesPerRow(XilinxConfigurationSpecification spec,
			int block) {
		int frames = 0;
	...
	public static int getMinorFromAddress(
			XilinxConfigurationSpecification spec, int address) {
		return (address & spec.getMinorMask()) >>> spec.getMinorBitPos();
	}

	public static int getNumberOfBRAMContentFrames(
			XilinxConfigurationSpecification spec) {
		int blockTypeNum = getBlockTypeNumber(spec,
				spec.getBRAMContentBlockType());
	...
	public static int getNumberOfColumns(XilinxConfigurationSpecification spec,
			int blockNum) {
		BlockType type = spec.getBlockTypes().get(blockNum);
	...
	public static int getNumberOfFramesPerBlock(
			XilinxConfigurationSpecification spec, int blockNum) {
		return getNumberOfFramesPerBlockTop(spec, blockNum)
				+ getNumberOfFramesPerBlockBottom(spec, blockNum);
	}

	public static int getNumberOfFramesPerBlockBottom(
			XilinxConfigurationSpecification spec, int blockNum) {
		return getNumberOfFramesPerBlockRow(spec, blockNum)
				* spec.getBottomNumberOfRows();
	}

	public static int getNumberOfFramesPerBlockRow(
			XilinxConfigurationSpecification spec, int blockNum) {
	...
	public static int getNumberOfFramesPerBlockTop(
			XilinxConfigurationSpecification spec, int blockNum) {
		return getNumberOfFramesPerBlockRow(spec, blockNum)
				* spec.getTopNumberOfRows();
	}

	public static int getRowFromAddress(XilinxConfigurationSpecification spec,
			int address) {
		return (address & spec.getRowMask()) >>> spec.getRowBitPos();
	}

	// Methods for extracting specific fields from a FAR address
	public static int getTopBottomFromAddress(
			XilinxConfigurationSpecification spec, int address) {
		return (address & spec.getTopBottomMask()) >>> spec
				.getTopBottomBitPos();
	}

	public static String toString(XilinxConfigurationSpecification spec,
			int address) {
		FrameAddressRegister far = new FrameAddressRegister(spec, address);
		return far.toString();
	}
	...
	/**
	 * Represents bits 21:19 (V4) or 23:21 (V5) of the Frame Address Register
	 * (FAR). Determines the type of block currently addressed. Virtex-4 Block
	 * types are: Logic blocks, CLB/IO/CLK/DSP/MGT: 000 Block RAM interconnect
	 * blocks: 001 Block RAM content blocks: 010 CFG_CLB blocks: 011 CFG_BRAM
	 * blocks: 100 A normal V4 bitstream does not include CFG_CLB or CFG_BRAM
	 * Virtex-5 Block types are: Logic blocks and interconnect,
	 * CLB/IO/CLK/DSP/BRAM interconnect: 000 Block RAM content blocks: 001
	 * Interconnect and Block Special Frames (used in partial reconfiguration):
	 * 010 Block RAM Non-Configuration Frames (not accessible by users): 011
	 */
	protected int blockType;
	/**
	 * Represents bits 13:6 (V4) or 14:7 (V5) of the Frame Address Register
	 * (FAR). Determines the column currently being addressed.
	 */
	protected int column;
	/**
	 * The configuration specification object that this class needs to know how
	 * to incrememnt and manipulate the FAR.
	 */
	...
	/**
	 * Represents bits 18:14 (V4) or 19:15 (V5) of the Frame Address Register
	 * (FAR). Determines the row currently being addressed.
	 */
	protected int row;
	...
	public int getClockRegionX() {
		...
		if (totalCols % 2 != 0) {
			Error.printError("Uneven number of Columns",
	...
	public int getConsecutiveAddress() {
		...
		// add addresses of rows in current block type that are *before* the
		// current row
		// but in the same FAR block type
		address += row * getFramesPerRow(configSpec, blockType);
		// Add addresses of columns in current row that are *before* the current
		// column
		for (i = 0; i < column; i++)
	...
	public String getFrameBlockSubTypeString() {
		return AbstractConfigurationSpecification.getBlockSubtype(configSpec,
				blockType, column);
	}

	public String getFrameBlockType() {
		return AbstractConfigurationSpecification.getBlockType(configSpec,
				blockType);
	}
	...
	public int getTotalNumberOfRows() {
		return configSpec.getTopNumberOfRows()
				+ configSpec.getBottomNumberOfRows();
	}
	...
	/**
	 * This function will increment the Frame Address Register (FAR) to the next
	 * address. The frame address register does not follow a sequential
	 * increment pattern. Instead, it follows a very particular pattern. There
	 * are smaller counters within the FAR that in the following order (from
	 * first to last): minor, column, row, top_bottom, and then type. See Xilinx
	 * UG071, Virtex-4 FPGA Configuration User Guide for more detail and Xilinx
	 * UG191, Virtex-5 FPGA Configuration User Guide.
	 * 
	 * @return true if the resulting FAR is valid. return false if increment is
	 *         invalid.
	 * 
	...
	public boolean incrementFAR() {
		...
		// If so, we can't increment and return false (bad FAR address - beyond
		// the end)
		if (blockType >= configSpec.getBlockTypes().size())
			return false;

		// Check to see if we have reached the last frame in a given
		// COnfifguration block.
		if (minor == getFramesPerConfigurationBlock(configSpec, blockType,
				column) - 1) {
			...
			if (column == getNumberOfColumns(configSpec, blockType) - 1) {
				...
				if ((top_bottom == 0 && row == configSpec.getTopNumberOfRows() - 1)
						|| (top_bottom == 1 && row == configSpec
								.getBottomNumberOfRows() - 1)) {
					row = 0;
					// If max top_bottom has been reached, set top_bottom to
					// zero and check the
					...
					if (top_bottom == 0) {
						...
						top_bottom = 0;
						// If the incremented type is greater than two, it is
						// invalid and we
						// will return false. Otherwise, we are finished and
						// return true.
						blockType++;
						if (blockType >= configSpec.getBlockTypes().size()) {
							return false;
	...
	public boolean isFrameTop() {
		String ret = AbstractConfigurationSpecification
				.getTopBottom(top_bottom);
		if (ret.equals("top")) {
			return true;
			...
			System.err
					.println("Failed to determine whether frame is in top or bottom");
			System.exit(1);
			return false;
		}
	}
	...
	public boolean isFrameTopFromAddress(int address) {
		String ret = AbstractConfigurationSpecification
				.getTopBottom(getTopBottomFromAddress(address));
		if (ret.equals("top")) {
			return true;
		} else if (ret.equals("bottom")) {
			return false;
	...
	public void setFARAddressFromConsecutiveAddress(int consecutiveAddress) {
		...
		address >= getFramesPerFARBlockType(configSpec, blockType); // See if
																	// address
																	// is large
																	// enough to
																	// skip
																	// current
																	// block
		address -= getFramesPerFARBlockType(configSpec, blockType++))
			// If so, reduce the address count and increment block count
			;
		...
		address >= getFramesPerConfigurationBlock(configSpec, blockType, column); address -= getFramesPerConfigurationBlock(
				configSpec, blockType, column++))
			;
		// detrmine minor
		minor = address;
	}
	...
	/**
	 * Level 0: hex address only Level 1: Single line string that decodes the
	 * FAR
	 */
	public String toString(int level) {
		...
		sb.append(", Row:" + row);
		sb.append(", Column:" + column + " (" + getFrameBlockSubType() + "),");
